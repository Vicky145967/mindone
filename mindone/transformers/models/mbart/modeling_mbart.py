...
class MBartLearnedPositionalEmbedding(mint.nn.Embedding):
    """
...
class MBartEncoder(MBartPreTrainedModel):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
...
class MBartModel(MBartPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Seq2SeqModelOutput, Tuple[ms.Tensor]]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
...
class MBartForConditionalGeneration(MBartPreTrainedModel, GenerationMixin):
    ...
    def _resize_final_logits_bias(self, new_num_tokens: int) -> None:
        ...
        if new_num_tokens <= old_num_tokens:
            ...
            extra_bias = mint.zeros((1, new_num_tokens - old_num_tokens), device=self.final_logits_bias.device)
